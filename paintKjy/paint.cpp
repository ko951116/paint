...
void drawCircle(void)
{
	...
	const float dtheta = 2.0 * PI / (float)resolution; //세타 =2파이/N
	float theta = 0.0;
	glPushMatrix();
	glTranslatef(posx, wh - posy, 0.0f);//위치이동
	...
	for (int i = 0; i < resolution; i++) {
		theta += dtheta; //증가된 세타 각도

		const float x = r*cos(theta);
		const float y = r*sin(theta);

		glVertex2f(x, y);//vertex들 나열
	}
...
void mouse(int btn, int state, int x, int y)
{
	...
	if (btn == GLUT_RIGHT_BUTTON && state == GLUT_DOWN) //오른쪽 버튼 눌렀을 때
	{
		glPushAttrib(GL_ALL_ATTRIB_BITS);
...
void display() //메뉴만드는것
{
	...
	double mx = ww - ww / 20;//원에서 사용
	double my = wh - (ww / 10 * 5.5);//원에서 사용
	double x[360], y[360];//원에서 사용
	double rad = 10;//원에서 사
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	glClearColor(0.8, 0.8, 0.8, 1.0);
	glColor3f(1.0, 1.0, 1.0);
	screen_box(ww - ww / 10, wh - ww / 10, ww / 10);//선
	glColor3f(1.0, 0.0, 0.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 2), ww / 10);//사각형
	glColor3f(0.0, 1.0, 0.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 3), ww / 10);//삼각형
	glColor3f(0.0, 0.0, 1.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 4), ww / 10);//점
	glColor3f(1.0, 1.0, 0.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 5), ww / 10);//텍스트
	glColor3f(0.0, 1.0, 1.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 6), ww / 10);//원
	glColor3f(1.0, 0.0, 1.0);
	screen_box(ww - ww / 10, wh - (ww / 10 * 7), ww / 10);//다각형
	glColor3f(0.0, 0.0, 0.0);
	glBegin(GL_LINES);//선
	glVertex2i(ww - (ww / 15), wh - ww / 20);
	glVertex2i(ww - (ww / 40), wh - ww / 20);
	glEnd();
	screen_box(ww - (ww / 40) * 3, wh - (ww / 10 * 1.8), ww / 20);//사각형
	...
	glEnd();
	glRasterPos2i(ww - ww / 10, wh - (ww / 10 * 4.7));//텍스트 A
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, 'A');
	shift = glutBitmapWidth(GLUT_BITMAP_9_BY_15, 'A');
	glRasterPos2i(ww - ww / 10 + shift, wh - (ww / 10 * 4.7));// 텍스트 B
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, 'B');
	shift += glutBitmapWidth(GLUT_BITMAP_9_BY_15, 'B');
	glRasterPos2i(ww - ww / 10 + shift, wh - (ww / 10 * 4.7));//텍스트 C
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, 'C');

	glBegin(GL_POLYGON); // 원
	r = ww/35;
	float i;
	double pi = 3.141592;
